...
export default function PayrollRunDetails() {
  ...
  type StaffGroup = {
    staff: EventStaffAssignment['staff'];
  ...
  const staffGroups = (run.items || []).reduce<Record<string, StaffGroup>>((acc, item) => {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          <button onClick={() => navigate('/payroll')} className="text-gray-500 dark:text-gray-400 hover:text-gray-700">
        ...
        <div className="flex space-x-3">
          <button className="inline-flex items-center px-3 py-2 border border-gray-300 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:bg-gray-700">
        ...
        <div className="px-4 py-5 sm:px-6 border-b border-gray-200 dark:border-gray-700">
          ...
          <dl className="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
            <div className="px-4 py-5 bg-gray-50 dark:bg-gray-700 shadow rounded-lg overflow-hidden sm:p-6">
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Total Staff</dt>
              <dd className="mt-1 text-3xl font-semibold text-gray-900 dark:text-white">{Object.keys(staffGroups).length}</dd>
            </div>
            <div className="px-4 py-5 bg-gray-50 dark:bg-gray-700 shadow rounded-lg overflow-hidden sm:p-6">
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Total Assignments</dt>
              <dd className="mt-1 text-3xl font-semibold text-gray-900 dark:text-white">{(run.items || []).length}</dd>
            </div>
            <div className="px-4 py-5 bg-gray-50 dark:bg-gray-700 shadow rounded-lg overflow-hidden sm:p-6">
              <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Total Payout</dt>
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-sm text-gray-500 dark:text-gray-400">{group.items.length} assignments</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-xl font-bold text-gray-900 dark:text-white">${group.total.toLocaleString()}</p>
                </div>
              </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200">
                    {group.items.map((item: EventStaffAssignment) => (
                      <tr key={item.id}>
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white">{item.event?.name}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{item.event?.date}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{item.role}</td>
                        <td className="px-3 py-2 whitespace-nowrap text-sm text-gray-900 dark:text-white text-right">${item.total_pay || item.pay_rate}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
