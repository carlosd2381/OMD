...
export default function UserManagementSettings() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      if (rolesData.length > 0) {
          ...
          setInviteRoles([defaultRole.id as UserRole]);
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">Manage team access, security, and sessions.</p>
          </div>
        </div>
        {activeTab === 'users' && (
          <button
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                className="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-md leading-5 bg-white dark:bg-gray-800 placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-primary focus:border-primary sm:text-sm"
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="shrink-0 h-10 w-10">
                          <span className="h-10 w-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 dark:text-gray-400 font-medium">
                            {user.name.charAt(0)}
                          </span>
                        </div>
                        <div className="ml-4">
      ...
      {/* Security Tab */}
      {activeTab === 'security' && (
        <div className="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6 max-w-3xl">
          <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-6">Password & Access Policies</h3>
          
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700">Password Rotation (Days)</label>
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">How often users must change their password.</p>
              <input
                type="number"
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-2">Automatically log out inactive users.</p>
              <input
                type="number"
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 text-sm">
                ...
                <p className="text-gray-500 dark:text-gray-400">Passwords must contain at least one symbol (!@#$%).</p>
              </div>
            </div>
            ...
            <div className="flex items-start">
              ...
              <div className="ml-3 text-sm">
                ...
                <p className="text-gray-500 dark:text-gray-400">Force all users to set up 2FA for their accounts.</p>
              </div>
            </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-end">
            <button
              ...
              className="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-red-700 bg-white dark:bg-gray-800 hover:bg-red-50"
            >
                  ...
                  <div className="px-4 py-4 sm:px-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex flex-col items-end">
                        <div className="text-sm text-gray-900 dark:text-white">{session.location}</div>
                        <div className="text-sm text-gray-500 dark:text-gray-400">
            ...
            <div className="space-y-4">
              <div className="flex items-center mb-4">
                <input
                  ...
                  className="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded"
                />
                <label htmlFor="manual_add" className="ml-2 block text-sm text-gray-900 dark:text-white">
                  ...
                  {availableRoles.map(role => (
                    <div key={role.id} className="flex items-center">
                      <input
                        ...
                        type="checkbox"
                        checked={inviteRoles.includes(String(role.id))}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setInviteRoles([...inviteRoles, String(role.id)]);
                          } else {
                            setInviteRoles(inviteRoles.filter(r => r !== String(role.id)));
                          }
                        }}
                        className="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded"
                      />
                  ...
                  {availableRoles.map(role => (
                    <div key={role.id} className="flex items-center">
                      <input
                        ...
                        type="checkbox"
                        checked={editRoles.includes(String(role.id))}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setEditRoles([...editRoles, String(role.id)]);
                          } else {
                            setEditRoles(editRoles.filter(r => r !== String(role.id)));
                          }
                        }}
                        className="focus:ring-primary h-4 w-4 text-primary border-gray-300 rounded"
                      />
